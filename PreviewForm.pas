...
uses
   Gopage, UCmdProcessor, AnsDatatypes;
...
begin
  ...
  ZoomBox.ItemIndex := 0;
  if (dm.GetOptionValue('PRINT PREVIEW CM', opt)) and (opt.Value = 'ON') then
...
begin
   ...
   Zoom := z;
   SB.Panels[1].text  := Format('%1.0n', [z * 100]) + '%';
end;
...
begin
   ...
   CheckEnable;
   ScrollBox1Resize(Nil);  {7.26} {make sure it gets sized}
end;
...
begin
   ...
   SB.Panels[0].text  := Format('Page %d of %d', [Val+1, tmp]);
   PB1.Invalidate;
   PB2.Invalidate;
end;
...
//   PrevPageBut.Enabled := CurPage > 0;
   ...
   NextPageSpeed.Enabled := CurPage+1 < MFPrinter.LastAvailablePage;
   PrevPageSpeed.Enabled := CurPage > 0;

   FirstPageSpeed.Enabled := NextPageSpeed.Enabled;
   LastPageSPeed.Enabled  := PrevPageSpeed.Enabled;
...
var
  ...
  Dummy: boolean;
begin
with PrintDialog1, MFPrinter do
  begin
