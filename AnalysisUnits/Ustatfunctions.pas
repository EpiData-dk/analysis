...
uses sysutils,Math,ansDataTypes, UepiDatatypes;

function EpiProportionCI(n,d: Epiint;var UL,LL: EpiFloat): string;
...
function EpiLRE(c,t: Epifloat; X: Integer = 0):Epifloat;
//function EpiRND(x: Epiint):EpiFloat;
function EpiRND(x: Epiint):EpiFloat;
function EpiRAN(x: Epiint):Epiint;
...
(*
 ...
 c : calaculated results  t: the answer supplied by NIST
 returns log relative error (KRE) as
 LRE=min(15, -log10(|c-t|/t) if |t|=0
 LRE=min(15, -log10(|c-t|) otherwise

 x: 0 or 1, used for testing purposes:
   lre(a,b, 0) return number of digits in common from left to right
     in numbers a and b.  Result returned in result variable $dp
   lre(a,b, 1) return number of digits as lre(a,b), but compare result
     to result variable $dp. Replace if new result is smaller than previous.
*)
...
begin
  if c=t then
    ...
    if (c=NA_FLOAT) or (t=NA_FLOAT) then
      result:=NA_FLOAT
    else
      if ABS(T)>0 THEN
        result:=max(0, -EpiLog10(ABS(C-T)/ABS(T)))
      else
        result:=max(0, -EpiLog10(abs(c)));
  result := min(15, result);
  result := round(result*10)/10;
  if (dm.Executor.FindVar('$dp') = nil) then
  begin
    dm.addresult('$dp', EpiTyFloat, 2000, 0, 0);
    dm.addresult('$dpdif', EpiTyFloat, 1E-15, 0, 0);
  end;
  dp := dm.Executor.FindVar('$dpdif').AsFloat;
  dm.addresult('$dp', EpiTyFloat, min(dm.Executor.FindVar('$dp').AsFloat, result),0,0) ;
  dm.addresult('$dpdif',EpiTyFloat,max(dp,(c-t)),0,0) ;
end;
