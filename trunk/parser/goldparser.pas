...
type
  TGoldParserMessage = (
    gpMsgEmpty,                      // Nothing
    gpMsgTokenRead,                  // Each time a token is read, this message is generated.
    gpMsgCommentLine,                // Each time a comment line is read, this message is generated
    gpMsgReduction,                  // When the engine is able to reduce a rule,
    ...
    gpMsgCommentError,               // The parser reached the end of the file while reading a comment.
                                     // This is caused when the source text contains a "run-away"
                                     // comment, or in other words, a block comment that lacks the
                                     // delimiter.
    gpMsgInternalError               // Something is wrong, very wrong
  );
...
//    FCaretNumber: Integer;
    ...
    function GetCaretNumber: Integer;
    function GetCurrentBytePosition: Integer;
    function GetLineNumber: Integer;
    procedure PrepareToParse;
    function RetrieveToken: TToken;
    function DiscardRestOfLine: UTF8String;
    function ParseToken(NextToken: TToken): TGoldParserResult;
  ...
  public
    ...
    function LoadCompiledGrammar(const FileName : UTF8String) : boolean; overload;
    ...
    function OpenTextString(Text: UTF8String): Boolean;
    function ReadTextString: UTF8String;
    ...
    function Parse: TGoldParserMessage;
    ...
    function Parameter(ParamName: UTF8String): UTF8String;
    ...
    property CurrentLineNumber: Integer read GetLineNumber;
    property CurrentCaretNumber: Integer read GetCaretNumber;
    property CurrentBytePosition: Integer read GetCurrentBytePosition;
...
function TGOLDParser.LoadCompiledGrammar(const FileName: UTF8String): boolean;
      // If the Compiled Grammar Table file is successfully loaded
      // the method returns True; otherwise False. This method must
      // be called before any Parse calls are made.
begin
...
function TGOLDParser.OpenTextString(Text: UTF8String): Boolean;
    // Opens the SourceString for parsing. If successful the method returns True; otherwise False.
begin
  Reset;
  FSource.Text := Text;
...
function TGOLDParser.ReadTextString: UTF8String;
begin
  Result := FSource.Text;
end;
...
begin
   ...
   Start := TToken.Create(1,1,1);
   Start.State := FInitialLALRState;
   Start.ParentSymbol := nil; //(FGrammarReader as TGrammarReader).StartSymbol;
   FStack.Push(Start);
end;

function TGOLDParser.GetCaretNumber: Integer;
begin
  result := FSource.CaretPos;
end;

function TGOLDParser.GetCurrentBytePosition: Integer;
begin
  result := FSource.ByteIndex;
end;

function TGOLDParser.GetLineNumber: Integer;
begin
  result := FSource.LinePos;
end;

function TGOLDParser.Parse: TGoldParserMessage;
    // Executes a parse.  When this method is called, the parsing engine
    // reads information from the source text (either a string or a file)
    // and then reports what action was taken. This ranges from a token
    // being read and recognized from the source, a parse reduction, or a type of error.
var Done : Boolean;
    ReadToken : TToken;
    ParseResult : TGoldParserResult;
begin
  Result := gpMsgEmpty;

  if (FActionTables.Count < 1) or
     (FDFA.Count < 1)
  then
    ...
    begin
      ...
      begin
        if (FInputTokens.Count = 0) then
          begin             //We must read a token
            ReadToken := RetrieveToken;

            if not Assigned(ReadToken) then
              begin
                ...
                if (FCommentLevel = 0) then
                  case ReadToken.Kind of
                    SymbolTypeCommentLine:
                      begin
                        ReadToken.DataVar := DiscardRestOfLine;
                        Result := gpMsgCommentLine;
                        Done := true;
                      end;

                    SymbolTypeWhitespace:
                      begin
                        if (ReadToken.Name = 'NewLine') then
                          FSource.NewLine;
                        FreeAndNil(ReadToken);
                      end;

                    // Do nothing for Comment start
          ...
          end // if (FInputTokens.Count = 0)
        else
          if (FCommentLevel > 0) then
            begin        //We are in a block comment
              ...
              if Assigned(ReadToken) then
                case ReadToken.Kind of
                  ...
                  SymbolTypeError        : begin
                                             if (ReadToken.DataVar = #10) or
                                                (ReadToken.DataVar = #13#10) or
                                                (ReadToken.DataVar = #13)
                                             then
                                               FSource.NewLine;
                                             FInputTokens.Pop;
                                             ReadToken.Free;
            ...
            begin
              ...
              begin
                case ReadToken.Kind of
                  ...
                  begin                                     //FINALLY, we can parse the token
...
function TGOLDParser.RetrieveToken: TToken;      //Symbol Index
var Done: Boolean;
    Found: Boolean;
    CurrentDFA: Integer;
    ...
    comp1, comp2: UTF8String;
    Target: Integer;
begin
  Result := TToken.Create(CurrentLineNumber, CurrentCaretNumber, CurrentBytePosition);
  ...
  if not FSource.Done then
  begin
    while not Done do begin
      Found := False;
      ch := FSource.GetNextCharacter;
      if (ch <> '') then
      begin
        n := 0;
        ...
        while (n < DFA[CurrentDFA].EdgeCount) and (not Found) do begin
          ...
          if FCompareMode = vbTextCompare then begin
            comp1 := UTF8UpperCase(comp1);
            comp2 := UTF8UpperCase(comp2);
          end;
      ...
      if Found then begin
        ...
        if LastAcceptState = -1 then
          ...
          begin
            Result.Free;
            Result := TToken.Create(CurrentLineNumber, CurrentCaretNumber - 1, CurrentBytePosition - 1);
            Result.ParentSymbol := FErrorSymbol;
            Result.DataVar := Ch; //FSource.ReadFromBuffer(1, True, True);
          end
        else
          begin
            FSource.PutCharacter(Ch);
...
function TGOLDParser.DiscardRestOfLine: UTF8String;
begin
  //Kill the current line - basically for line comments
  result := FSource.ReadLine;
end;

function TGOLDParser.ParseToken(NextToken: TToken): TGoldParserResult;
var Index: Integer;
    RuleIndex: Integer;
    CurrentRule: TRule;
    Head: TToken;
...
function TGOLDParser.Parameter(ParamName: UTF8String): UTF8String;
    // Returns a string containing the value of the specified parameter.
    // The ParameterName is the same as the parameters entered in the
    // grammar's description. These include: Name, Version, Author, About,
    // Case Sensitive and Start Symbol. If the name specified is invalid,
