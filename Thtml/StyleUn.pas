...
{$endif}
  ...
  public
    bgColor: TColor;
    tmHeight, tmDescent, tmExternalLeading, tmMaxCharWidth, tmCharset: integer;
    CharExtra: integer;    
  ...
  public
    ...
    procedure Inherit(Source: TProperties);
...
procedure TProperties.Inherit(Source: TProperties);
{copy the properties that are inheritable}
var
  I: PropIndices;
begin
for I := Low(I) to High(I) do
  if (Source.PropTag = 'span') and (I <> BorderStyle) then   {Borderstyle already acted on} 
    Props[I] := Source.Props[I]
  else
    case I of
      MarginTop..LeftPos:
      ...
      Position, PageBreakBefore, PageBreakAfter, PageBreakInside, Display:
          ;  {do nothing}
      else
        Props[I] := Source.Props[I];
      end;
...
if VarType(Props[LineHeight]) = varString then
  ...
  if Code = 0 then   {a numerical entry with no 'em', '%', etc.  Use the new font height}
    ...
    Result := LengthConv(Props[LineHeight], True, EmSize, EmSize, ExSize, 0);
  end
else Result := -1;     
end;
