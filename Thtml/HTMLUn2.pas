...
type
  ...
  Symb = (
    ...
    TopMarginSy, LeftMarginSy, LabelSy, LabelEndSy, EolSy);  
  ...
  public
    ...
    constructor Create(ASym: Symb; AValue: integer; Const NameStr, ValueStr: String);
    destructor Destroy; override;
  end;
...
function GetImageAndMaskFromStream(Stream: TMemoryStream;
        var Transparent: Transparency; var AMask: TBitmap): TPersistent; 
function KindOfImage(Start: Pointer): ImageType;
procedure FormControlRect(Canvas: TCanvas; X1: integer;
           Y1: integer; X2: integer; Y2: integer; Raised, PrintMonoBlack, Disabled: boolean);
...
procedure RaisedRectColor(SectionList: TFreeList; Canvas: TCanvas; X1: integer;
           ...
           W: integer);
function EnlargeBitmap(Bitmap: TBitmap; W, H: integer): TBitmap;  
...
procedure DrawBorder(Canvas: TCanvas; ORect, IRect: TRect; C: htColorArray;
            S: htBorderStyleArray; BGround: TColor);
...
uses
  ...
  {$ifndef NoPNG}
     PngImage,
     //PngImage1, Changed by Torsten - Aug 9 2006
  {$endif}
...
if W = 1 then  {this looks better in Print Preview}
  ...
  DrawBorder(Canvas, Rect(X1-W+1, Y1-W+1, X2+W, Y2+W), Rect(X1+1, Y1+1, X2, Y2), Colors,
              htStyles(bssSolid, bssSolid, bssSolid, bssSolid), clNone);
  end;
end;
...
constructor TAttribute.Create(ASym: Symb; AValue: integer; Const NameStr, ValueStr: String);
begin
inherited Create;
Which := ASym;
Value := AValue;
...
for I := 0 to Size-1 do
  if not (P[I] in [FmCtl, ImgPan, BrkCh]) then {ImgPan and FmCtl used to mark images, form controls}
    if Leng < SizeM1 then
      begin
      if Integer(P[I]) = 160 then // special NO-BRAEK-space symbol (unicode)
        Buffer[Leng] := WideChar(32)
      else
        Buffer[Leng] := P[I];
      Inc(Leng);
      end;
...
try
  ...
  try
    ...
    if IT in [Gif, Gif89] then
      ...
      begin
      Result := GetImageAndMaskFromStream(Stream, Transparent, Mask) as TBitmap;  
      Mask.Free;
      end;
  finally
    Stream.Free;
...
var
  ...
  {$ifndef NoPng}
  PI: TPNGObject;
//  PI: TPngImage;  Changed by Torsten - Aug 9 2006
  {$endif}
begin
Result := Nil;
AMask := Nil;
...
try
  ...
  {$ifndef NoPng}
    ...
    if IsTransparentPNG(Stream, Color) then  {check for transparent PNG}
      Transparent := TPng;   
    PI := TPNGObject.Create;  //TPngImage.Create;  Changed by Torsten - Aug 9 2006
    try
...
if not Assigned(FBitmap) then
  ...
  FMask.Width := Width;
  FMask.Height := Height;
  FMask.Canvas.Draw(0, 0, Self);
  FMask.Canvas.CopyMode := cmDstInvert;
  FMask.Canvas.Draw(0, 0, Self);
...
{----------------EnlargeBitmap}
function EnlargeBitmap(Bitmap: TBitmap; W, H: integer): TBitmap;   
{enlarge 1 pixel bitmaps for tiling}
begin
Result := TBitmap.Create;
...
procedure DrawOnePolygon(Canvas: TCanvas; P: BorderPointArray; Color: TColor; Side: integer);
...
try
  for I := 0 to 3 do
    if S[I] in [bssSolid, bssInset, bssOutset] then
      ...
      case S[I] of
        bssSolid:
          DrawOnePolygon(Canvas, Bnd, Color, I);     
        bssInset:
          begin
          if I in [0,1] then
