...
FormControlRect(Canvas, X1, Y1, X1+LB.Width, Y1+LB.Height, False, MasterList.PrintMonoBlack, False);
Canvas.Brush.Style := bsClear;
Canvas.Font := LB.Font;
H2 := Abs(Canvas.Font.Height);
SetTextAlign(Canvas.handle, TA_Left+TA_Top);
ARect := Rect(X1+4, Y1+4, X1+LB.Width-8, Y1+LB.Height-8);    
if UnicodeControls then
  for I := LB.TopIndex to IntMin(LB.Items.Count-1, LB.TopIndex+LBSize-1) do    
    {$Warnings Off}
    ExtTextOutW(Canvas.Handle, X1+4, Y1+4+(I-LB.TopIndex)*H2, ETO_CLIPPED, @ARect,
       PWideChar(LB.Items[I]), Length(LB.Items[I]), nil)
    {$Warnings On}
else
  for I := LB.TopIndex to IntMin(LB.Items.Count-1, LB.TopIndex+LBSize-1) do
    Canvas.TextRect(ARect, X1+4, Y1+4+(I-LB.TopIndex)*H2, LB.Items[I]);
end;
...
FormControlRect(Canvas, X1, Y1, X1+CB.Width, Y1+CB.Height, False, MasterList.PrintMonoBlack, False);
Canvas.Brush.Style := bsClear;
Canvas.Font := CB.Font;
SetTextAlign(Canvas.handle, TA_Left+TA_Top);
...
with ThtMemo(FControl) do
  ...
  SetTextAlign(Canvas.handle, TA_Left+TA_Top);
  ARect := Rect(X1+4, Y1+4, X1+Width-8, Y1+Height-8);  
  if UnicodeControls then
    for I := 0 to IntMin(Lines.Count-1, Rows-1) do  
      {$Warnings Off}
      ExtTextOutW(Canvas.Handle, X1+4, Y1+4+I*H2, ETO_CLIPPED, @ARect,
         PWideChar(Lines[I]), Length(Lines[I]), nil)
      {$Warnings On}
  else
    for I := 0 to IntMin(Lines.Count-1, Rows-1) do
      Canvas.TextRect(ARect, X1+4, Y1+4+I*H2, Lines[I]);
  end;
end;
