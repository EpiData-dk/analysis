...
type
  ...
  private
    ...
    BRect: TRect;
    MargArray: TMarginArray;
    procedure DrawTheBorder(Canvas: TCanvas; YOffSet: integer);
    end;

  InlineRec = class
  private
    StartB, EndB, IDB: integer;
    MargArray: TMarginArray;
    end;
  ...
  public
    ...
    //EpiData hack SM (v8) +JL (v9)
var InTableCell : boolean;
...
if MargArray[Width] <> IntNull then
  ...
  if MargArray[Width] = Auto then
    ...
    begin
    PercentWidth := True;
    SpecWidth := MulDiv(MargArray[Width], 100, 200);
    end
  else
...
if MargArray[Height] <> IntNull then
  ...
  if MargArray[Height] = Auto then
    ...
    begin
    PercentHeight := True;
    SpecHeight := MulDiv(MargArray[Height], 100, 200);
    end
  else
...
{$ifndef NoMetafile}
  ...
  Result := ThtMetaFile(Image).Bitmap;
{$endif}
end;
    ...
    if PercentWidth then     
      ...
      if SpecHeight <> 0 then
        if PercentHeight then
          ObjHeight := MulDiv(AvailableHeight, SpecHeight, 100)
...
with Canvas do
    ...
    case ObjAlign of
      ATop, ALeft, ARight:
        ...
        RaisedRect(ParentSectionList, Canvas, X, TopY+VSpaceT,
                                      X+ObjWidth-1, TopY+ObjHeight-1+VSpaceT, False, 1);
        end;
      AMiddle:
        ...
        RaisedRect(ParentSectionList, Canvas, X, MiddleAlignTop,
                         X+ObjWidth-1, MiddleAlignTop+ObjHeight-1, False, 1);
        end;
      ABottom, ABaseline:
        begin
        if FAlt <> '' then
        ...
        RaisedRect(ParentSectionList, Canvas, X, YBaseLine-ObjHeight-VSpaceB,
                                      X+ObjWidth-1, YBaseLine-VSpaceB-1, False, 1);
        end;
      end;
    end;
  if HasBlueBox then
  ...
  if Assigned(MyFormControl) and MyFormControl.Active or FO.Active then    
    ...
    case ObjAlign of   {draw focus box}
      ALeft, ARight, ATop:
          Canvas.DrawFocusRect(Rect(X, TopY+VSpaceT, X+ImageWidth, TopY+VSpaceT+ImageHeight));  
      AMiddle:
          Canvas.DrawFocusRect(Rect(X, MiddleAlignTop, X+ImageWidth, MiddleAlignTop + ImageHeight));
      ABottom, ABaseline:
          Canvas.DrawFocusRect(Rect(X, YBaseLine-ImageHeight-VSpaceB, X+ImageWidth, YBaseLine-VSpaceB));
      end;
...
with ThtEdit(FControl) do
  ...
  Canvas.Font := Font;
  H2 := Abs(Font.Height);
  FormControlRect(Canvas, X1, Y1, X1+Width, Y1+Height, False, MasterList.PrintMonoBlack, False);
  SetTextAlign(Canvas.handle, TA_Left);
  SetBkMode(Canvas.Handle, Windows.Transparent);  
  Canvas.Brush.Style := bsClear; 
  ARect := Rect(X1+4, Y1, X1+Width-8, Y1+Height); 
  if UnicodeControls then
    {$Warnings Off}
    ExtTextOutW(Canvas.Handle, X1+4, Y1+(Height-H2)div 2, ETO_CLIPPED, @ARect,
      PWideChar(Text), Length(Text), nil)
    {$Warnings On}
  else
    Canvas.TextRect(ARect, X1+4, Y1+(Height-H2)div 2, Text);
  end;
end;
...
with ThtEdit(FControl) do
  ...
  if not PercentWidth then
    if (FWidth >= 10) then   
      Width := FWidth
    else Width := Canvas.TextWidth('0')*EditSize+20    
  else
...
with ThtButton(FControl) do
  ...
  if not MonoBlack then                 
    ...
    Canvas.Brush.Style := bsClear;
    Canvas.Font := Font;
    FormControlRect(Canvas, X1, Y1, X1+Width, Y1+Height, True, MasterList.PrintMonoBlack, False);
...
with TCheckBox(FControl) do
  begin
  FormControlRect(Canvas, X1, Y1, X1+Width, Y1+Height, False, MasterList.PrintMonoBlack, Disabled);
  if Checked then
    with Canvas do
      begin
      Pen.Color := clBlack;
...
with TFormRadioButton(FControl) do
  ...
  OnClick := RadioClick;
  Tabstop := True;     {so an empty group will intercept a tab}
  end;
end;
...
begin
  ...
  while I < TheCount do
    ...
    try
      SB :=  TSectionBase(Items[I]);
      Tmp := SB.DrawLogic(Canvas, 0, Y+H, 1000, 1000, Width, AHeight, BlHt, IMgr, Sw, Curs);
      H := H+Tmp;
...
var                            
  ...
  TheBitmap, TheMask, NewBitmap, NewMask: TBitmap;
  ...
  repeat  {tile BGImage in the various dc's}
    XX := X;
    repeat
      TiledImage.Canvas.Draw(XX, Y, Bitmap);
...
if (OW = 1) or (OH = 1) then   
  begin  {in case a 1 pixel bitmap is being tiled}
  NewBitmap := EnlargeBitmap(TheBitmap, X2-X+1, Y2-Y+1);
  try
    if Assigned(TheMask) then
      NewMask := EnlargeBitmap(TheMask, X2-X+1, Y2-Y+1)
    else NewMask := Nil;
    try
...
if (VarType(MargArrayO[Width]) = VarString) and (Pos('%', MargArrayO[Width]) > 0) then  
  NewWidth := IntMin(NewWidth, AWidth - MiscWidths);  
...
with ParentSectionList do
  if Printing and (Positioning <> posAbsolute) then
    ...
    else if Self is TTableBlock then
...
if (BT <= ARect.Bottom) and (BB >= ARect.Top) then
  ...
  try
    if NeedDoImageStuff and (BGImage.Image <> DefBitmap) and (BGImage.Image <> ErrorBitmap) then
      begin
...
if ShowImages and not BitmapLoaded and (BitmapName <> '') then
  ...
  if not Assigned(BackgroundBitmap) then
    ...
    {$ifndef NoMetafile}
      ...
      begin
      BackgroundBitmap := ThtMetaFile(TmpResult).Bitmap;
      BackgroundMask := ThtMetaFile(TmpResult).Mask;
      end
    {$endif}
    else
      begin
...
with InlineList do
  ...
  if Start then
    ...
    with IR do
      begin
      StartB := SIndex;  {Source index for border start}
    ...
    for I := Count-1 downto 0 do   {the record we want is probably the last one}
      ...
      if Prop.ID = IR.IDB then     {check the ID to make sure}
        begin
        IR.EndB := SIndex; {the source position of the border end}
        Break;
        end;
      end;
  end;
...
for I := 0 to Count-1 do
  with InlineRec(Items[I]) do
    begin
    StartB := Owner.FindDocPos(StartB, False);
    EndB := Owner.FindDocPos(EndB, False);
    if StartB = EndB then
  ...
  if BorderStyle <> bssNone then
    ...
    if (BrdTop=1) and (BrdRight=1) and (BrdBottom=1) and (BrdLeft=1) and
      ...
      DrawBorder(Canvas, Rect(BL, BT, BR, BB), Rect(PL, PT, PR, PB),
                       Colors, Styles, MargArray[BackgroundColor]);
    end
...
Begin
if ThisCycle = ParentSectionList.CycleNumber then
  GoTo 2;
...
2:
ThisCycle := ParentSectionList.CycleNumber;
TotalMaxWidth := 0;  TotalMinWidth := 0;
for I := 0 to NumCols-1 do
  begin
  Inc(TotalMaxWidth, MaxWidths[I]);
...
if UseAbsolute and (WidthAttr = 0) then
  ...
  GetWidths(Canvas, Min, Max, WidthAttr);
...
if Delta > 0 then
  ...
  Inc(Widths[0], -Delta);   {tack it on anywhere}
end;
...
var
  I, J, K, H, Span, TotalHt, TotalMinHt, TotalDesHt, AddOn,
  ...
  for J := 0 to Rows.Count-1 do
    with TCellList(Rows[J]) do
      ...
      H := DrawLogic1(Canvas, Widths, Span, CellSpacing, IntMax(0,AHeight-Rows.Count*CellSpacing),
                       Desired, IsSpeced, Mr) + CellSpacing;
...
var
  I, J, K, N, Span,
  TotalMaxWidth, TotalMinWidth, D, W, DS, MaxDS, MaxI,
  Total, TotalPC, Residual, NewResidual, W1, W2, NewTotal, LastNewTotal: integer;
  More, Mr, HasPercents, UsesPercents, Done, TmpSpec: boolean;
...
try
  ...
  else if ParentSectionList.InLogic2 and (ParentSectionList.TableNestLevel <= 4) then
    FindRowHeights(Canvas, AHeight);
      ...
      for I := 0 to Count-1 do
        if Assigned(Items[I]) then
          ...
          begin
            Cell.CopyToClipboard;
             if I <> Count-1 then
               ParentSectionList.CB.AddText(#9, 1)
             else
               ParentSectionList.CB.AddText(^M^J, 2);
          end;
//EpiData hack SM (v8) +JL (v9)
   InTableCell:=false;
end;
...
if (Length(BuffS) > 0) and (BuffS[Length(BuffS)] = ' ') then 
  Remove(Length(BuffS));
...
var
  ...
  Tmp, Tmp1: integer;
  Obj: TFloatingObj;
  TopY, AccumImgBot, TotHt, HtRef: integer;   
...
var
  ...
  while Cnt > 0 do
    ...
    if LR.FirstDraw and Assigned(LR.BorderList) then
      for K := 0 to LR.BorderList.Count-1 do  {may be several inline borders}
        ...
        if (Start-Buff = BR.BStart) then     
          ...
          BR.bRect.Left := CP.x;
          BR.bRect.Bottom := Y-Descent+Desc{-1};
          if BR.MargArray[BackgroundColor] = clNone then
            BR.MargArray[BackgroundColor] := FO.TheFont.BGColor;
          end
        else if (Start-Buff = BR.BEnd) and (BR.bRect.Right = 0) then
          BR.bRect.Right := CP.x  {this is it's end}
          ...
          BR.bRect.Bottom := IntMax(BR.bRect.Bottom, Y-Descent+Desc{-1});
          end;
        end;
    ...
    if J2 = -1 then
            ...
            for K := 0 to LR.BorderList.Count-1 do
              ...
              if (Start-Buff >= BR.BStart) and (Start-Buff <= BR.BEnd) then     
                ...
                if (Start-Buff = BR.BStart) then     
                  ...
                  BR.bRect.Top := ToPP;
                  BR.bRect.Left := CP.x + TImageObj(Obj).HSpaceL;
      ...
      {the following is needed to make the DrawFocusRect work correctly for some
       reason.  It must be located before the GetCurrentPositionEx call}
      if FO.Active then
  ...
  with LineRec(Lines[I]) do
    ...
    if Assigned(BorderList) then  {draw any borders found in this line}
      for K := 0 to BorderList.Count-1 do
        ...
        BR.DrawTheBorder(Canvas, YOffSet);
        end;
...
for I := 0 to Lines.Count-1 do
  ...
  //EpiData hack SM (v8) +JL (v9)
  if (MySelE > Len) and not InTableCell then
    ParentSectionList.CB.AddTextCR('', 0);
end;
...
procedure BorderRec.DrawTheBorder(Canvas: TCanvas; YOffSet: integer);
var
  IRect, ORect: TRect;
begin
IRect := BRect;
