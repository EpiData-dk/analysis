...
{$i htmlcons.inc}
{*********************************************************}
{*    // notice EpiData specific HACK in htmlsubs*}
{*********************************************************}
...
if FSectionList.ShowImages and Assigned(Bitmap) then
  ...
  DrawBackground2(ACanvas, ARect, X, Y, X2, Y2, Bitmap, Mask, BW, BH, AColor);
  end
else
  begin  {no background image, show color only}
...
try
  ...
  try
    Curs := 0;
    DocHeight := CopyList.DoLogic(Canvas, 0, FormatWidth, 300, 0, Dummy, Curs);
...
try
  ...
  try
    ...
    Result.Height := Height;
    Curs := 0;
    DocHeight := CopyList.DoLogic(Result.Canvas, 0, FormatWidth, 0, 300, Dummy, Curs);
...
var
  ...
  try
    XL := MLeft;
    XR := MLeft + W;
    Event(Self, HFViewer, FPage, PrintList.PageBottom + HTop > VPixels, XL, XR, Done);  {call event handler}
      ...
      with vwP do
        ...
        while (FPage <= ToPage) and not Done do
          ...
          if FPage >= FromPage then
            ...
            Canvas.Rectangle(MLeft, PrintList.PageBottom-HTop+TopPixels+2,
                                           W + MLeft+1, TopPixels+H);
            if (htPrintBackground in FOptions) and (PrintList.PageBottom < HTop + H) then  
              begin  {need to reprint background in whited out area}
              hrgnClip1 := CreateRectRgn(MLeftPrn, MulDiv(PrintList.PageBottom-HTop+TopPixels+2, P3, P2),
...
var
  ...
  var
    ...
    DocHeight, XL, XR: integer;
  begin
  if NoOutput or not Assigned(Event) then
    Exit;
  try
    XL := MLeft;
    XR := MLeft + W;
    ...
    try
      HFCopyList.Printing := True;
      Curs := 0;
      DocHeight := HFCopyList.DoLogic(MFPrinter.Canvas, 0, XR-XL, 300, 0, Dummy, Curs);
      if not Footer then
        begin  {Header}
        YOrigin := 0;
        Ht := TopPixels;
  ...
  try
    ...
    try
      with MFPrinter do
        ...
        hrgnClip := CreateRectRgn(MLeftPrn, IntMax(0, TopPixelsPrn-1),  
...
begin
  // Torsten - 27-feb-2006 - to prevent cached imaged when deleted!
  //FSectionList.BitmapList.BumpAndCheck;
  FSectionList.BitmapList.PurgeCache;
end;
...
var
  ARect: TRect;
  Bitmap, Mask, NewBitmap, NewMask: TBitmap;
  PRec: PtPositionRec;
  BW, BH, X, Y, X2, Y2, IW, IH, XOff, YOff: integer;
...
with FViewer do
  ...
  if FSectionList.ShowImages and Assigned(Bitmap) then
    ...
    if (BW = 1) or (BH = 1) then   
      ...
      NewBitmap := EnlargeBitmap(Bitmap, X2-X, Y2-Y);
      try
        if Assigned(Mask) then
          NewMask := EnlargeBitmap(Mask, X2-X, Y2-Y)
        else NewMask := Nil;
        try
          DrawBackground(ACanvas, ARect, X, Y, X2, Y2, NewBitmap, NewMask, NewBitmap.Width, NewBitmap.Height, Self.Color);
        finally
          NewMask.Free;
          end;
      finally
