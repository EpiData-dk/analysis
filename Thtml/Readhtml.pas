...
Const
  ...
  ResSy : array[1..MaxRes] of Symb =
     (htmlSy, TitleSy, BodySy, HeadSy, BSy, ISy, HeadingSy, EmSy, StrongSy,
      ...
      WrapSy, MetaSy, PanelSy, WbrSy, LinkSy, ColSy);
...
while GetAttribute(Sym, SymStr, AttrStr, L) do
    Attributes.Add(TAttribute.Create(Sym, L, SymStr, AttrStr));
  ...
  for I := 0 to Attributes.Count-1 do
    with TAttribute(Attributes[I]) do
      case Which of
        ...
        CharSetSy:
          if TranslateCharSet(Name, CharSet) then
...
I := Pos(#0, DocS);
if I > 0 then
  SetLength(DocS, I-1); {file has a problem}
Buff := PChar(DocS);
...
if (Length(DocS) > 3) and (DocS[1] = #$EF) and (DocS[2] = #$BB) and (DocS[3] = #$BF) then
  ...
  if (Pos(#$1b'$@', DocS) > 0) or (Pos(#$1b'$B', DocS) > 0) then
    begin
...
var
  ...
  begin
    ...
    else if (I < 256) and not ForceUnicode then
      Buffer.Add(Char(I), SaveIndex)
    else
    begin
      ...
      '#': // Numeric value.
        ...
        try
          ...
          repeat
            ...
            GetCh;
            Inc(N);
          until not (Ch in ['A'..'Z', '0'..'9']) or (N > 10);
